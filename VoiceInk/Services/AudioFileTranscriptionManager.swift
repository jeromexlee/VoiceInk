...
class AudioTranscriptionManager: ObservableObject {
    ...
    @Published var transcriptionProgress: Double = 0.0  // Progress from 0.0 to 1.0
    @Published var realtimeTranscriptionText: String = ""  // Real-time transcription display
                    ...
                    text = try await localTranscriptionService!.transcribe(
                        ...
                        onProgress: { [weak self] progress in
                            Task { @MainActor in
                                ...
                                self?.messageLog = "Transcribing... \(Int(progress * 100))%"
                            }
                        },
                        onNewSegment: { [weak self] segmentText in
                            Task { @MainActor in
                                // Append new segment to real-time transcription with newline
                                if self?.realtimeTranscriptionText.isEmpty == true {
                                    self?.realtimeTranscriptionText = segmentText
                                } else {
                                    self?.realtimeTranscriptionText += "\n" + segmentText
                                }
                            }
                        }
                    )
