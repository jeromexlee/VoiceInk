...
class WhisperState: NSObject, ObservableObject, AVAudioRecorderDelegate {
    ...
    @Published var showDebugPopup = false
    
    // Debug logging helpers
    private func addDebugLog(_ message: String) {
        // Always log to console for system debugging, but only add to UI logs if debug mode is enabled
        logger.notice("🔍 DEBUG: \(message)")
        
        // Only add to debug logs array (for UI popup) if debug logging is enabled
    ...
    private func transcribeAudio(_ url: URL) async {
        // Clear previous debug logs and start fresh
        await MainActor.run {
            clearDebugLogs()
        ...
        guard let model = currentTranscriptionModel else {
            ...
            await MainActor.run {
                addDebugLog("❌ ERROR: No transcription model selected")
                // Only show debug popup if debug logging is enabled
                if UserDefaults.standard.bool(forKey: "DebugLoggingEnabled") {
                    showDebugPopup = true
                }
            }
        ...
        await MainActor.run {
            addDebugLog("🤖 Using transcription model: \(model.displayName) (Provider: \(model.provider.rawValue))")
        }
        ...
        do {
            ...
            await MainActor.run {
                addDebugLog("🔧 Using transcription service: \(model.provider == .local ? "LocalTranscriptionService" : "CloudTranscriptionService")")
            }
            ...
            await MainActor.run {
                addDebugLog("✅ Transcription completed successfully, length: \(text.count) characters")
                addDebugLog("📝 Transcription text: '\(text)'")
            }
            ...
            // --- Enhancement Service Debug Logging ---
            await MainActor.run {
                addDebugLog("🔍 Starting enhancement service checks...")
                if let enhancementService = enhancementService {
                    addDebugLog("✅ Enhancement service exists")
                    addDebugLog("🔑 Enhancement service isConfigured: \(enhancementService.isConfigured)")
                    addDebugLog("🎛️ Enhancement service isEnhancementEnabled: \(enhancementService.isEnhancementEnabled)")
                    
                    if let aiService = enhancementService.getAIService() {
                        addDebugLog("🤖 AI Service selectedProvider: \(aiService.selectedProvider.rawValue)")
                        addDebugLog("🔐 AI Service isAPIKeyValid: \(aiService.isAPIKeyValid)")
                        addDebugLog("📡 AI Service currentModel: \(aiService.currentModel)")
                    } else {
                        addDebugLog("❌ AI Service is nil")
                    }
                } else {
                    addDebugLog("❌ Enhancement service is nil")
                }
            }
            
            if let enhancementService = enhancementService, enhancementService.isConfigured {
                await MainActor.run {
                    addDebugLog("✅ Enhancement service is configured, running prompt detection...")
                }
                let detectionResult = promptDetectionService.analyzeText(text, with: enhancementService)
                promptDetectionResult = detectionResult
                await promptDetectionService.applyDetectionResult(detectionResult, to: enhancementService)
                await MainActor.run {
                    addDebugLog("✅ Prompt detection completed")
                }
            } else {
                await MainActor.run {
                    addDebugLog("❌ Skipping prompt detection - enhancement service not configured")
                }
            }
            ...
            if let enhancementService = enhancementService,
               ...
               enhancementService.isConfigured {
                await MainActor.run {
                    addDebugLog("🚀 ALL ENHANCEMENT CONDITIONS MET - Starting AI enhancement!")
                    addDebugLog("   ✅ enhancementService exists")
                    addDebugLog("   ✅ isEnhancementEnabled: \(enhancementService.isEnhancementEnabled)")
                    addDebugLog("   ✅ isConfigured: \(enhancementService.isConfigured)")
                }
                
                do {
                    // Don't cancel enhancement if we've already started it - all conditions were met
                    await MainActor.run {
                        addDebugLog("🚀 Enhancement process starting - ignoring cancellation requests")
                    }
                    
                    let textForAI = promptDetectionResult?.processedText ?? text
                    await MainActor.run {
                        addDebugLog("📤 Sending text to AI for enhancement: '\(textForAI)'")
                    }
                    
                    let enhancedText = try await enhancementService.enhance(textForAI)
                    
                    await MainActor.run {
                        addDebugLog("✅ AI enhancement completed successfully!")
                        addDebugLog("📥 Enhanced text: '\(enhancedText)'")
                    }
                    ...
                    await MainActor.run {
                        addDebugLog("❌ AI enhancement failed with error: \(error.localizedDescription)")
                        addDebugLog("⚠️ Using original text instead")
                    }
                    let newTranscription = Transcription(
                        text: originalText,
                        duration: actualDuration,
                ...
                await MainActor.run {
                    addDebugLog("❌ ENHANCEMENT SKIPPED - Conditions not met:")
                    if enhancementService == nil {
                        addDebugLog("   ❌ enhancementService is nil")
                    } else {
                        addDebugLog("   ✅ enhancementService exists")
                        addDebugLog("   🎛️ isEnhancementEnabled: \(enhancementService?.isEnhancementEnabled ?? false)")
                        addDebugLog("   🔑 isConfigured: \(enhancementService?.isConfigured ?? false)")
                    }
                }
                let newTranscription = Transcription(
                    text: originalText,
            ...
            await MainActor.run {
                addDebugLog("🎉 Transcription process completed successfully!")
                addDebugLog("📋 Final text: '\(text.trimmingCharacters(in: .whitespacesAndNewlines))'")
                
                // Only show debug popup if debug logging is enabled
                if UserDefaults.standard.bool(forKey: "DebugLoggingEnabled") {
                    addDebugLog("🔍 Debug session completed - showing popup")
                    showDebugPopup = true
                } else {
                    addDebugLog("🔍 Debug session completed (popup disabled)")
                }
            }
            ...
            await MainActor.run {
                addDebugLog("❌ TRANSCRIPTION FAILED: \(error.localizedDescription)")
                // Only show debug popup if debug logging is enabled
                if UserDefaults.standard.bool(forKey: "DebugLoggingEnabled") {
                    showDebugPopup = true
                }
