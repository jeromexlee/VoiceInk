...
struct AudioTranscribeView: View {
    ...
    private var processingView: some View {
        VStack(spacing: 16) {
            VStack(spacing: 12) {
                if transcriptionManager.processingPhase == .transcribing && transcriptionManager.transcriptionProgress > 0 {
                    // Show determinate progress bar during transcription
                    VStack(spacing: 8) {
                        ProgressView(value: transcriptionManager.transcriptionProgress, total: 1.0)
                            .frame(width: 250)
                        
                        Text("\(Int(transcriptionManager.transcriptionProgress * 100))% complete")
                            .font(.caption)
                            .foregroundColor(.secondary)
                    }
                } else {
                    // Show indeterminate progress for other phases
                    ProgressView()
                        .scaleEffect(0.8)
                }
                ...
                if !transcriptionManager.messageLog.isEmpty {
            ...
            if transcriptionManager.processingPhase == .transcribing && !transcriptionManager.realtimeTranscriptionText.isEmpty {
                VStack(alignment: .leading, spacing: 8) {
                    HStack {
                    ...
                    ScrollView {
                        ScrollViewReader { proxy in
                            VStack(alignment: .leading, spacing: 4) {
                                Text(transcriptionManager.realtimeTranscriptionText)
                                    .textSelection(.enabled)
                                    .font(.body)
                                    .foregroundColor(.primary)
                            ...
                            .onChange(of: transcriptionManager.realtimeTranscriptionText) { oldValue, newValue in
